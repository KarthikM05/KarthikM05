# Hi, I'm Karthik 👋

**Program Manager | MBA in Business Analytics (AI/ML) | Engineering + IT deployments + AI project/program management & airline strategy analytics**

---

## 🚀 About Me
- 🎓 MBA in Business Analytics (AI/ML) from Christ University  
- 🎓 BE in Electronics & Communication Engineering  
- 💼 3.5+ years in IT deployments (RFID, POS, kiosks, global rollouts)  
- 💼 Associate Program Manager in AI automation (healthcare, insurance, industrial domains)  
- ✈️ Airline strategy project during MBA sparked my interest in aviation & aerospace program management  
- 🌍 Experienced in global coordination, compliance frameworks, and operational governance  

---

## 🛠️ Skills
- **Project/Program Management:** Governance, risk registers, compliance checks, stakeholder management  
- **Analytics & BI:** Power BI, Tableau, SQL, Python, NLP, scenario modeling  
- **Operations:** SLA metrics, dashboards, inventory/claims tracking, ERP/CRM familiarity (SAP, Oracle, SFDC)  
- **Domains:** IT deployments, AI automation, airline strategy, healthcare, insurance  

---

## 📂 Featured Interests
- Building governance templates & dashboards for operational excellence  
- Exploring aviation program management opportunities  
- AI/ML applications in regulated industries  
- Creating cryptic, stylish captions blending culture, nostalgia & tech  

---

## 📫 Connect With Me
- [LinkedIn](https://www.linkedin.com/in/karthikmugila/)  
- [Resume/Portfolio](https://drive.google.com/file/d/1No6eJ7eq3J9GaUTIlmUhLLrceu36pzcf/view?usp=sharing)  
- Email: karthikmugila3@gmail.com  

---
